// Crop one element on the live site and on mine, by selector, at a width.
import { chromium } from 'playwright';
const [,, refSel, mineSel, w, name, path] = process.argv;
const b = await chromium.launch();
const ctx = await b.newContext({ reducedMotion: 'reduce', deviceScaleFactor: 2 });
for (const [label, base, sel] of [['ref', 'https://storefaq.io', refSel], ['mine', 'http://localhost:4321', mineSel]]) {
