...
for (const y of [0, 600, 1500, 4000, 9000, 17000]) {
  ...
  const ok = r.active===m.active && r.lit===m.lit && r.dim===m.dim && r.linePos===m.linePos;
  console.log(`scrollY ${String(y).padStart(6)}  ref active=${r.active} lit=${r.lit} dim=${r.dim} line=${r.linePos}@${r.lineTop}`
    + `   mine active=${m.active} lit=${m.lit} dim=${m.dim} line=${m.linePos}@${m.lineTop}  ${ok?'✓':'✗'}`);
}
